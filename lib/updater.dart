...
// is public, so the releases API needs no token. (Same as BodyComp / Pantry.)
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// UPDATE SHEET — the full update UI. Opened from the slim launch banner or
// by long-pressing the top edge (see main.dart). Chromeless by default.
// ═══════════════════════════════════════════════════════════════════════

enum _State { idle, checking, upToDate, available, downloading, error }

class UpdateSheet extends StatefulWidget {
  /// If provided, the sheet opens already knowing an update is available
  /// (the launch-time auto-check found one) and skips straight to it.
  final ReleaseInfo? preloaded;
  const UpdateSheet({super.key, this.preloaded});
  @override
    ...
    switch (_s) {
      ...
      case _State.downloading:
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 8),
          const Text("The installer will open automatically when it's ready.",
        ...
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Update available — v${r.version}',
              style: const TextStyle(
                  fontSize: 14, fontWeight: FontWeight.w700, color: kAccent)),
      ...
      case _State.error:
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(_msg, style: const TextStyle(fontSize: 13, color: kWarn)),
